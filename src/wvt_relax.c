...
#pragma omp parallel for reduction(+:errMean,nIn) reduction(max:errMax)
        ...
        sprintf(wvt_filename, "test_%d", it);
...
#pragma omp parallel for shared(delta, hsml, P) \
        ...
        for (int ipart = 0; ipart < nPart; ipart++) {
            ...
            for (int i = 0; i < ngbcnt; i++) { // neighbour loop
                ...
                // There seems to be an off-by-one type situation where
                // this seed with 1000 particles breaks at particle 307
                if ( (dx == 0) && (dy == 0) && (dz == 0))
                    continue;
                ...
                // if (ipart == 307 || ipart == 306)
                // {
                //     printf("%d %d %g %g %g \n", ipart, jpart,
                //         (float) (delta[0][ipart] * boxsize),
                //         (float) (delta[1][ipart] * boxsize),
                //         (float) (delta[2][ipart] * boxsize));
                //     printf("%g %g %g %g %g %g %g\n\n", step, hsml[ipart], wk, dx, dy, dz, r);
                // }
            }
        }
...
static float global_density_model(const int ipart)
{
    ...
    double rho_i = Gas_Density_Profile(dx, dy, dz);
